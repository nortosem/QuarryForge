"""Fossil Timeline Process module.
...
from quarryforge.exception import fossil_exception, model_exception
...
def parse_timeline(timeline_output: str) -> model.FossilTimeline:
    """Parse the Timeline to prepare updates.
    ...
    (
        ...
        timeline.hash_pattern(),
        timeline.date_pattern(),
        timeline.author_pattern(),
        timeline.comment_pattern(),
        timeline.branch_pattern(),
        timeline.tags_pattern(),
        timeline.phase_pattern(),
        timeline.change_pattern(),
    )
    for commit_info in all_commits_raw:
        ...
        for entry in commit_info.split('\n'):
            if match := uuid_re.match(entry):
                commit_data[timeline.HASH_KEY] = match.group(timeline.HASH_KEY)
            elif match := date_re.match(entry):
                commit_data[timeline.DATE_KEY] = match.group(timeline.DATE_KEY)
            elif match := author_re.match(entry):
                commit_data[timeline.AUTHOR_KEY] = match.group(
                    timeline.AUTHOR_KEY
                )
            elif match := comment_re.match(entry):
                commit_data[timeline.COMMENT_KEY] = match.group(
                    timeline.COMMENT_KEY
                )
            elif match := branch_re.match(entry):
                commit_data[timeline.BRANCH_KEY] = match.group(
                    timeline.BRANCH_KEY
                )
            elif match := tags_re.match(entry):
                commit_data[timeline.TAGS_KEY] = match.group(
                    timeline.TAGS_KEY
                ).split(', ')
