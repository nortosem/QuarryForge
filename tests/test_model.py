...
from quarryforge.model import FossilRepo, FossilCommit, FossilTimeline
from quarryforge.exception import model_exception, meta_exception
...
def mock_commit(mock_commit_data: dict[str, Any]) -> FossilCommit:
    ...
    with patch('quarryforge.util.model_util.viable_fossil_commit',
...
class TestFossilRepo:
    ...
    def test_init_success(
        self,
        mock_viable_repo: MagicMock,
        mock_repo_paths: Tuple[Path, Path]
    ) -> None:
...
class TestFossilCommit:
    ...
    def test_init_success(
        self,
        mock_viable_commit: MagicMock,
        mock_commit_data: dict[str, Any]
    ) -> None:
    ...
    def test_properties_and_get_hash(
        self,
        mock_commit: FossilCommit,
        mock_commit_data: dict
    ) -> None:
...
class TestFossilTimeline:
    ...
    def test_init_reverses_order(self, mock_commit: FossilCommit) -> None:
        ...
        commit1 = mock_commit
        commit2_data = {
            'uuid': '2', 'date': '2', 'author': 'a2', 'comment': 'c2'
        }
        with patch(
            'quarryforge.util.model_util.viable_fossil_commit',
            return_value=tuple(commit2_data.values()) + (None, None, None, None)
        ):
            commit2 = FossilCommit(**commit2_data)

        timeline = FossilTimeline(commits=[commit2, commit1]) # Newest to oldest
